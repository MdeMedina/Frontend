...
export const Card = ({ title, icon, children, className }: CardProps) => (
  <div className={`bg-white rounded-sm p-4 border border-black/[0.05] shadow-sm hover:shadow-md hover:border-black/[0.1] transition-all duration-300 ${className}`}>
...
export const Field = ({ label, value }: { label: string; value: any }) => (
  <div className="bg-gray-50/50 p-2 rounded-sm border border-black/[0.03] text-center hover:bg-gray-100/50 hover:border-black/[0.06] transition-all duration-200 group">
...
export const DiffField = ({ label, current, requested, isDate = false, formatDate }: any) => {
  ...
  return (
    <div className={`p-2 rounded-sm border transition-all duration-200 hover:shadow-sm ${isChanged ? 'bg-amber-50/50 border-amber-200 hover:bg-amber-100/50 hover:border-amber-300' : 'bg-gray-50/30 border-black/[0.03] hover:bg-gray-100/30 hover:border-black/[0.06]'}`}>
